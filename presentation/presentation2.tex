...
\begin{frame}{CMSIS-RTOS: Sincronización — el problema}
    \begin{columns}
        \begin{column}{0.52\textwidth}
            \begin{itemize}
                \item Varios threads se ejecutan \textbf{concurrentemente}
                      y pueden acceder al mismo recurso hardware o software.
                \item Sin control: los accesos se \textbf{entrelazan} y el
                      recurso queda en estado inconsistente.
                \item El SO proporciona mecanismos para garantizar el acceso
                      \textbf{secuencial}.
            \end{itemize}
            \vspace{6pt}
            \begin{tabular}{ll}
...
\begin{frame}[fragile]{CMSIS-RTOS: Colas (Message Queue)}
    \begin{itemize}
        \item Patrón \textbf{Productor/Consumidor}: un thread (o ISR)
              produce datos y otro los consume de forma segura.
...
% ============================================================
\section{Configuración y herramientas}
% ============================================================
...
\begin{frame}{Watch Window RTX — Depuración del RTOS}
    \begin{columns}
        \begin{column}{0.52\textwidth}
            \includegraphics[width=\textwidth]{presentation/watchwindowRTX.png}
        \end{column}
...
\begin{frame}{CMSIS-RTOS: Consideraciones de diseño}
    \begin{itemize}
        \item Diseñar la estructura antes de codificar: cada periférico
              gestionado por un módulo con su thread, señales, colas y
              estructuras de datos.
        \item Las ISRs serán cortas: la HAL llama a las callbacks; desde
              ellas solo \texttt{Set}/\texttt{Release}, nunca bucles ni
              funciones bloqueantes. Verificar qué funciones del SO se
              pueden usar en su interior.
        \item Prestar especial atención a la sincronización entre threads.
        \item Reducir al mínimo el uso de variables globales: pasar datos
              mediante colas o semáforos.
    \end{itemize}
\end{frame}

\begin{frame}{Buenas prácticas}
    \begin{itemize}
        \item \textbf{Dimensionar la pila correctamente}: usar la Watch
              Window para comprobar el uso real. Un desbordamiento de pila
              produce comportamiento impredecible.
        \item \textbf{No bloquear en callbacks de timer}: el
              \texttt{osRtxTimerThread} es compartido por todos los timers;
              bloquearlo afecta a todos ellos.
        \item \textbf{Cuidado con las prioridades}: un thread de alta
              prioridad que nunca se bloquea impide la ejecución del resto.
        \item \textbf{Mutex para exclusión mutua, semáforo para
              señalización}: no intercambiarlos — el mutex tiene propietario,
              el semáforo no.
        \item \textbf{Nombrar los objetos del SO} con \texttt{osThreadAttr\_t},
              \texttt{osMutexAttr\_t}\ldots: aparecen en la Watch Window y
              facilitan enormemente la depuración.
    \end{itemize}
\end{frame}
...
\begin{frame}{¿Qué mecanismo de sincronización usar?}
    \begin{table}[H]
    \centering
    \begin{tabular}{|p{2.6cm}|p{4.0cm}|p{2.8cm}|}
    \hline
    ...
    \hline
    \textbf{Thread Flags}
        & Señal a un thread concreto (se conoce el \texttt{tid})
        & Sí (\texttt{Set}) \\
    \hline
    \textbf{Event Flags}
...
\begin{frame}[fragile]{Mutex — Exclusión Mutua}
    \begin{itemize}
        ...
        \item El thread que lo adquiere es el \textbf{propietario único};
              debe liberarlo cuando termina de usar el recurso.
